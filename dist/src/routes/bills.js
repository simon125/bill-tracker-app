"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __spreadArrays = (this && this.__spreadArrays) || function () {
    for (var s = 0, i = 0, il = arguments.length; i < il; i++) s += arguments[i].length;
    for (var r = Array(s), k = 0, i = 0; i < il; i++)
        for (var a = arguments[i], j = 0, jl = a.length; j < jl; j++, k++)
            r[k] = a[j];
    return r;
};
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (Object.hasOwnProperty.call(mod, k)) result[k] = mod[k];
    result["default"] = mod;
    return result;
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
// node modules
var express = __importStar(require("express"));
var express_validator_1 = require("express-validator");
// local modules
var Bill_1 = __importDefault(require("../models/Bill"));
var auth_1 = __importDefault(require("../middleware/auth"));
var router = express.Router();
// @route  GET api/v1/bills
// @desc   Get all users bills
// @access Private
router.get('/', [auth_1.default], function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var bills, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Bill_1.default.find({ user: req.user.id })];
            case 1:
                bills = _a.sent();
                res.status(200).json(bills);
                return [3 /*break*/, 3];
            case 2:
                error_1 = _a.sent();
                console.error(error_1);
                res.status(500).send('Server Error');
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// @route  GET api/v1/bills/:id
// @desc   Get users bill by id
// @access Private
router.get('/:id', [auth_1.default], function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var bill, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Bill_1.default.findById(req.params.id)];
            case 1:
                bill = _a.sent();
                res.status(200).json(bill);
                return [3 /*break*/, 3];
            case 2:
                error_2 = _a.sent();
                console.error(error_2);
                res.status(500).send('Server Error');
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// @route  POST api/v1/bills
// @desc   Add bill
// @access Private
var validateBill = [
    express_validator_1.check('expenseName', 'Expense name is required')
        .not()
        .isEmpty(),
    express_validator_1.check('totalPrice', 'Total price is required')
        .not()
        .isEmpty(),
    express_validator_1.check('transactionDate', 'Transaction date is required')
        .not()
        .isEmpty()
];
router.post('/', __spreadArrays([auth_1.default], validateBill), function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var errors, _a, expenseName, totalPrice, transactionDate, newBill, bill, error_3;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                errors = express_validator_1.validationResult(req);
                if (!errors.isEmpty()) {
                    return [2 /*return*/, res.status(400).json({ errors: errors.array() })];
                }
                _a = req.body, expenseName = _a.expenseName, totalPrice = _a.totalPrice, transactionDate = _a.transactionDate;
                _b.label = 1;
            case 1:
                _b.trys.push([1, 3, , 4]);
                newBill = new Bill_1.default({
                    expenseName: expenseName,
                    totalPrice: totalPrice,
                    transactionDate: transactionDate,
                    user: req.user.id
                });
                return [4 /*yield*/, newBill.save()];
            case 2:
                bill = _b.sent();
                res.status(200).json(bill);
                return [3 /*break*/, 4];
            case 3:
                error_3 = _b.sent();
                console.error(error_3);
                res.status(500).send('Server Error');
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
router.put('/:id', [auth_1.default], function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, expenseName, totalPrice, transactionDate, billFields, bill, error_4;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, expenseName = _a.expenseName, totalPrice = _a.totalPrice, transactionDate = _a.transactionDate;
                billFields = {};
                if (expenseName) {
                    billFields.expenseName = expenseName;
                }
                if (totalPrice) {
                    billFields.totalPrice = totalPrice;
                }
                if (transactionDate) {
                    billFields.transactionDate = transactionDate;
                }
                _b.label = 1;
            case 1:
                _b.trys.push([1, 4, , 5]);
                return [4 /*yield*/, Bill_1.default.findById(req.params.id)];
            case 2:
                bill = _b.sent();
                if (!bill) {
                    return [2 /*return*/, res.status(401).json({ msg: 'Bill not found' })];
                }
                if (bill.user.toString() !== req.user.id) {
                    return [2 /*return*/, res.status(401).json({ msg: 'Not authorized' })];
                }
                return [4 /*yield*/, Bill_1.default.findByIdAndUpdate(req.params.id, { $set: billFields }, { new: true })];
            case 3:
                bill = _b.sent();
                res.status(200).json(bill);
                return [3 /*break*/, 5];
            case 4:
                error_4 = _b.sent();
                console.error(error_4);
                res.status(500).send('Server Error');
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); });
// @route  DELETE api/v1/bills/:id
// @desc   Delete bill
// @access Private
router.delete('/:id', [auth_1.default], function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var bill, error_5;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, Bill_1.default.findById(req.params.id)];
            case 1:
                bill = _a.sent();
                if (!bill) {
                    return [2 /*return*/, res.status(404).json({ msg: 'Contact not found' })];
                }
                if (bill.user.toString() !== req.user.id) {
                    return [2 /*return*/, res.status(401).json({ msg: 'Not authorized' })];
                }
                return [4 /*yield*/, Bill_1.default.findByIdAndRemove(req.params.id)];
            case 2:
                _a.sent();
                res.status(200).json({ msg: 'Bill removed' });
                return [3 /*break*/, 4];
            case 3:
                error_5 = _a.sent();
                console.error(error_5);
                res.status(500).send('Server Error');
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
exports.default = router;
